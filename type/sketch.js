...
function preload() {
  font = loadFont("Stase-Regular.ttf");
}
...
function draw() {
  ...
  if (rotationX == 0 && rotationY == 0) {
    // desktop
    rx = constrain(
      map(mouseY, 0, height, PI * 0.01, -PI * 0.01),
      -PI / 10,
      PI / 10
    );
    ry = constrain(
      map(mouseX, 0, width, -PI * 0.01, PI * 0.01),
      -PI / 10,
      PI / 10
    );
  } else {
    // mobile
    rx = constrain(
      map(rotationX, PI / 2, -PI / 2, -PI * 0.005, PI * 0.005),
      -PI / 15,
      PI / 15
    );
    ry = constrain(
      map(rotationY, -PI / 2, PI / 2, -PI * 0.005, PI * 0.005),
      -PI / 15,
      PI / 15
    );
  }
  ...
  world.gravity.x =
    cos(frameCount / 50 + random(-0.01, 0.01)) * 0.00001 * w +
    rotationY * 0.001;
...
function setupColors() {
  ...
  // ! FG
  poolSfc = color(257, 79, 99);
  
  // ! BG
  poolEfc = color(240, 21, 11);

  // ! Tiles
  poolSc = color(227, 4, 87);

  shadowBoxCol = color(0, 0, 0, 0);
...
function randColors() {
  ...
  shapeTextCol = color(0, 0, random(1) < 0.5 ? 0 : 100);
...
function updateSurface() {
  ...
  for (let y = 0; y <= h; y += yTileInc) {
    for (let x = 0; x <= w; x += xTileInc) {
      tilePoints[idx].z =
        noise(x * 0.002 + frameCount * 0.002, y * 0.002 + frameCount * 0.002) *
          30 -
        15; // z move range
      idx++;
    }
...
function setupSurface() {
  for (let y = 0; y < numTileRows + 1; y++) {
    // +1 for boundary points
